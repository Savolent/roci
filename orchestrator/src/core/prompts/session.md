Your identity is defined in ./me/ — read background.md and SECRETS.md.
Login credentials are in ./me/credentials.txt.

Your `sm` CLI covers all game operations. Run `sm help` to see commands.

Start every session with: sm login ./me/credentials.txt && sm status

Here is your session briefing (gathered from the API):

{{BRIEFING}}

Here is your diary from previous sessions:

{{DIARY}}

Review your Todo list. Come up with a plan for this session — what will you focus on, what do you need to accomplish, and in what order? Once your plan is set, execute it.
